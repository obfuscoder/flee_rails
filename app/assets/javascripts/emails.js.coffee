...
$(document).on 'click', '#new_email input[type=radio][data-ref]', ->
...
get_choice = (option) ->
  $('#new_email input[name="email[' + option + ']"]:checked').val()
filter_for_option = (array, option) ->
  choice = get_choice(option)
  event = $('#email_' + option + '_event').val()
...
filter_function = (choice) ->
  ...
  passthru_filter
$(document).on 'change', '#new_email [data-update]', ->
