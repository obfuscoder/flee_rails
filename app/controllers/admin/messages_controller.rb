  ...
  class MessagesController < AdminController
    ...
    def invitation
      ...
      sellers.each do |seller|
        SellerMailer.invitation(seller, @event, host: request.host).deliver_later
      end
    ...
    def send_mails_and_redirect
      ...
      @event.reservations.each do |reservation|
        if block_given?
          SellerMailer.send(action, reservation, yield(reservation), host: request.host).deliver_later
        else
          SellerMailer.send(action, reservation, host: request.host).deliver_later
        end
      end
