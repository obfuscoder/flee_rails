...
Rails.application.config.sorcery.configure do |config|
  ...
  config.user_config do |user|
    ...
    # Default: `nil`
    #
    # user.reset_password_mailer = NotificationMailer
...
# we need to overwrite this to incorporate the client scope
module Sorcery
  module Adapters
    class ActiveRecordAdapter
      ...
      class << self
        ...
        def find_by_credentials(credentials)
          ...
          obj = if Thread.current[:current_client]
                  Thread.current[:current_client].users
                else
                  @klass
                end
          obj.where(relation).first
        end
      end
    end
  end
